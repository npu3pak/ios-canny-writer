...
- (void)removeButtonPressed:(id)removeButton {
    _removeActionSheet = [[UIActionSheet alloc] initWithTitle:@"Удалить изображение?"
                                                     delegate:self
...
- (void)addButtonPressed:(id)removeButton {
    _addActionSheet = [[UIActionSheet alloc] initWithTitle:NSLocalizedString(@"actionSheetAddTitle", @"Прикрепить изображение")
                                                  delegate:self
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (actionSheet == _addActionSheet) {
        if (buttonIndex == 0)
            [_delegate addImageFromCamera];
        else if (buttonIndex == 1)
            [_delegate addImageFromLibrary];
    } else if (actionSheet == _removeActionSheet) {
        if (buttonIndex == 0 && [_delegate respondsToSelector:@selector(removeImageWithIndex:)]) {
            [_delegate removeImageWithIndex:_currentPageIndex];
        }
    }
}
...
- (void)performLayout {
    ...
    // Right - Action
    if (_actionButton && (!(!hasItems && !self.navigationItem.rightBarButtonItem) || _showAddButton)) {
...
- (void)loadAdjacentPhotosIfNecessary:(id <MWPhoto>)photo {
    ...
    if (page) {
        ...
        if (_currentPageIndex == pageIndex) {
            if (pageIndex > 0) {
                // Preload index - 1
                id <MWPhoto> photoAtIndex = [self photoAtIndex:pageIndex - 1];
