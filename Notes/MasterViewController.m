...
- (void)addRecord:(id)sender {
    ...
    Record *newRecord = [NSEntityDescription insertNewObjectForEntityForName:[entity name] inManagedObjectContext:context];
...
- (void)configureCell:(UITableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    ...
    NSSet *history = [record history];
    for (NSManagedObject *historyItem in history) {
        NSString *title = [historyItem valueForKey:@"title"];
        NSLog(@"%@", title);
    }
    cell.textLabel.text = [[record valueForKey:@"creationDate"] description];
}
